...
M.build_systems = {
  ...
  cmake = {
    debug = {
      ...
      build = function ()
        local build_dir = 'build/' .. M.target_platform .. '/debug'
        return 'mkdir -p ' .. build_dir .. ' && cmake -S. -B ' .. build_dir .. ' && cmake --build ' .. build_dir .. '&& find build -type f -exec touch {} +'
      end,
      run = function()
    ...
    release = {
      ...
      build = function()
        local build_dir = 'build/' .. M.target_platform .. '/release'
        return 'mkdir -p ' .. build_dir .. ' && cmake -S. -B ' .. build_dir .. ' && cmake --build ' .. build_dir .. ' --config release && find build -type f -exec touch {} +'
      end,
      run = function()
